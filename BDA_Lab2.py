...
df_max.saveAsTextFile("Q1_result-max")
df_min.saveAsTextFile("Q1_result-min")
...
df2_2 = data_frame.filter((data_frame.date>'1950')&(data_frame.date<'2014'))\
                ...
                .groupBy('year','month').count()\
                .sort(desc("count"))\
df2.saveAsTextFile("Q2_result-1")
df2_2.saveAsTextFile("Q2_result-2")
...
prec_df = spark_session.read\
                        .option("delimiter", ";")\
                        .csv('precipitation-readings.csv',header=True, inferSchema=True).toDF("Station_number","date","time","Precipitation","Quality")\
                        .cache()
...
df4_2.saveAsTextFile("Q4-result")
...
station_df = spark_session.read\
                        .option("delimiter", ";")\
                        .csv('stations-Ostergotland.csv',header=False, inferSchema=True)\
