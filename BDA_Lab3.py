...
#collect results for each time-point in a list
    pred_results_add.append(est_temp_add)
    pred_results_prod.append(est_temp_prod)
#kernel addition results
pred_add = sc.parallelize(pred_results)
pred_results_sum.coalesce(1).saveAsTextFile("Lab3_results/sum")
#kernel multiplication results
pred_add = sc.parallelize(pred_results_prod)
pred_results_prod.coalesce(1).saveAsTextFile("Lab3_results/prod")
